function [OUTPUTS, Force_blade, Torque_blade, stall_percentage, V_tip] = ...
    compute_blade_force_testFunc(R_b_i, V_b, v_induced, angular_velocity, SIM, OUTPUTS, ROTOR, BLADE, ATMOSPHERE)
...
for i = 0:1:BLADE.No_elements
    ...
    R_b_e = [cosd(ang_rot)   0   sind(ang_rot); 
                  0          0        0      ;
            -sind(ang_rot)   0    cosd(ang_rot)];

    R_e_b = transpose(R_b_e);

    dF_e = [dD(i+1); 0; dL(i+1)];
