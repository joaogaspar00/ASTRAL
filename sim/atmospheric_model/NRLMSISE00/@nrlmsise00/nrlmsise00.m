classdef nrlmsise00 < handle
    
    properties(Constant)
        
        % physical constants
        RGAS = 8.314; %46261815324;  % [J?K?1?mol?1] Universal Gas Constant
        Boltzmann = 1.3806e-23; %1.380649e-23; % [J/K]
        
        % for spline calculations of temperature and density at altitude,
        % these arrays of spline node altitude are unchanging in nrlmsise00
        % altitude of spline nodes for lower stratosphere and troposphere
        MN3 = 5; % [count] number of nodes
        ZN3 = [32.5,20.0,15.0,10.0,0.0]'; % [km] geopotential altitude of nodes
        % lower mesosphere and upper stratosphere
        MN2 = 4; % [count] number of nodes
        ZN2 = [72.5,55.0,45.0,32.5]'; % [km]  geopotential altitude of nodes
        % upper mesosphere and thermosphere
        % note: in the fortran model, ZN1(1) = 120.0, but then is set to
        % PDL(16,2) which is 123.4350 and also a constant.  For this
        % implementation the value 120 is replaced with PDL(16,2) and set
        % as a contant to make all the arrays as constants
        MN1 = 5; % [count] number of nodes
        ZN1 = [ 123.4350, 110.0, 100.0, 90.0, 72.5]';  % [km]  geopotential altitude of nodes    
    ...
    properties(GetAccess=public, SetAccess=private)
        ...
        % the regions are
        % <n> Description
        %  3  surface through lower stratosphere (0 to 32.5 km)
        %  2  upper stratosphere through lower mesosphere (32.5 to 72.5 km)
        %  1  upper mesosphere through thermosphere (72.5 to 123.435 km)
        %
        %  TN<n>  - [K] (MN<n>x1) Temperature at nodes  (scenario variable)
        TN1 = zeros(5,1);
        TN2 = zeros(4,1);
        TN3 = zeros(5,1);
        %
        %  TGN<n> - [K] (2x1) Temperature gradients at end nodes (scenario variable)
        TGN1 = zeros(2,1);
        TGN2 = zeros(2,1);
        TGN3 = zeros(2,1);
    end
    ...
    properties(GetAccess=public, SetAccess=private)
        ...
        % Earth size, shape, and surface gravity
        % PARMB common data in the fortran code
        % set these latitude-dependent values with:
        %    [this.RE,this.GSURF] = nrlmsise00.GLATF(LAT);
        GSURF = 0; % [cm/s^2 or gal] acceleration due to gravity at this latitude
        RE = 0; % [km] effective radius of the earth at this latitude
                     
        % /DMIX/
        DM28 = 0.0; % this value is shared
        
        % stored values to speed up the calculations
        LAST_ALT_GTD7 = 99999.0; % [km]
        LAST_ALT_GTS7 = -999.9;  % [km]
        ...
        SWC = [0 ones(1,24)]';
        ISW = 0;
        SAV = zeros(25,1); % last versions
                
        % the LPOLY common data
        PLG = zeros(9,4);
        CTLOC;
        STLOC;
        C2TLOC;
    ...
    %% *user-input values*
    %  DAY   - [DDD] day of the year (1 to 366)
    %  SEC   - [seconds] UTC
    %  ALT   - [km] altitude 
    %  GLAT  - [degN] geodetic latitude
    %  GLONG - [degE] geodetic longitude
    %  STL   - [hours] local apparent solar time
    %  F107A - [sfu] 3-month average F10.7 flux value
    %  F107  - [sfu] daily F10.7 flux value of the previous day
    %  AP    - [2nT] magnetic index
    %          if (this.SW(9) ~= -1) it is a single (daily) value
    %          if (this.SW(9) ==  1) then it is an array of 7 values:
    %          AP(1) DAILY AP
    ...
    %          AP(6) average of eight 3 HR AP indices from
    %                12 to 33 HRS prior to current time
    %          AP(7) average of eight 3 HR AP indices from
    %                36 to 59 HRS prior to current time   
    %  MASS - MASS NUMBER (ONLY DENSITY FOR SELECTED GAS IS
    %         CALCULATED.  MASS 0 IS TEMPERATURE.  MASS 48 FOR ALL.
    %         MASS 17 IS Anomalous O ONLY.)
    % note:
    % (1) for this matlab implementation, it is being hardwired to always
    % use a 7x1 array for AP, but filling the same value if the user only
    % specifies one value
    % (2) 1 sfu = 10^22 W m^2 Hz^1 = solar flux units
    properties(GetAccess=public, SetAccess = private)
        ...
        % variables should be consistent (LOCALSOLARTIME=SEC/3600+GLONG/15).
        ...
        % want all the species if MASS=48
        MASS = 48;
        
        % flag if the values have changed
        % this is managed via storing the last values and calling function
        % VTST7 in the fortran implementation.  Here, changing values
        % triggers this value to be set
        flagVTST7 = 1; % have the geophysical parameters changed?
        %                0 - unchanged
        %                1 - changed
        
        IMR = 0; % METSEL metric system 0=CGS, 1=KGM        

    end
    
    %% *Profile Output*
    properties(GetAccess=public, SetAccess = public)
        ProfileAlt; % [km] array for generating a profile
        D_He;
        D_O;
        D_N2
        D_O2;
    ...
    methods
        ...
        function set.ALT(this,value)
            if (0 <= value && value <= 1000)
                this.ALT = value;
            else
                error('[ERROR] User-input out of range: 0 <= ALT <= 1000 [km]');
            end
        end
        
        function set.GLAT(this,value)
            if (-90 <= value && value <= 90)
                this.GLAT = value;
            else
                error('[ERROR] User-input out of range: -90 <= GLAT <= 90 [degN]');
            end
        end
        
        function set.GLONG(this,value)
            if (-180 <= value && value <= 360)
                this.GLONG = wrapTo360(value);
            else
                error('[ERROR] User-input out of range: 0 <= GLONG <= 360 [degE]');
            end
        end

        function set.F107A(this,value)
            if (0 <= value && value <= 1000)
                this.F107A = value;
            else
                disp('[WARN] User-input out of reasonable values: 0 <= F107A <= 300 [sfu]');
                this.F107A = value;
            end
        end

        function set.F107(this,value)
            if (0 <= value && value <= 1000)
                this.F107 = value;
            else
                disp('[WARN] User-input out of reasonable values: 0 <= F107 <= 300 [sfu]');
                this.F107 = value;
            end
        end        
        
        function set.AP(this,value)
            switch numel(value)
                case 1
                    this.AP = value*ones(7,1);
                case 7
                    this.AP = value;
                otherwise
                    disp('[ERROR] User-input out of reasonable values: 0 <= AP <= 300 [2nT]');
            end
        end        
                
        function set.IMR(this,value)
            switch true
                ...
                otherwise
                    disp('[ERROR] User-input IMR: select unit system 0=CGS, 1=KGM');
            end           
        end
        
        function [this] = nrlmsise00(IDAY,SEC,ALT,GLAT,GLONG,F107A,F107,AP,MASS)
            this.init(IDAY,SEC,ALT,GLAT,GLONG,F107A,F107,AP,MASS);
        end
            
        function [this] = init(this,IDAY,SEC,ALT,GLAT,GLONG,F107A,F107,AP,MASS)
            
            % geographic location
            this.ALT = ALT;     % [km] geopotential altitude
            this.GLAT = GLAT;   % [degN] geodetic latitude
            this.GLONG = GLONG; % [degE] geodetic longtiude
            % new latitude - update the gravity and effective radius
            [this.RE,this.GSURF] = nrlmsise00.GLATF(this.GLAT);
            
            this.DAY = IDAY; % [days] day of year
            this.SEC = SEC;  % [sec] UTC
            % estimated value (can be overwritten by user)
            % for now, this is hardwired to be 'correct'
            % in model, it can be different and thus account for the
            % equation of time or do different analysis
            this.LOCALSOLARTIME = this.SEC/3600 + this.GLONG/15; % [hr]
            
            this.MASS = MASS; % [g/mol] 48=all molecules
            
            % solar wind loading
            this.F107 = F107; % [sfu]
            this.F107A = F107A; % [sfu]
            this.DF  = F107 - F107A; % [sfu]
            this.DFA = F107A - 150.0; % [sfu]
            this.AP = AP; % [nT]
            
            this.updateGLAT();          
            this.updateLOCALSOLARTIME();
            this.LAST_ALT_GTD7 = 99999.0; % [km]
            this.LAST_ALT_GTS7 = -999.9;  % [km]
            
            % hardwired for now
            this.IMR = 1;
            this.TSELEC(ones(25,1));
        end

        function [this] = updateLOCALSOLARTIME(this)
            % sin/cos of local hour angle(s)
            HR = pi/12.0; % [rad/hr] pi/12 = 0.2618
        ...
        function [atmTable] = calculateProfile(this,AltArray_km)
            if (~exist('AltArray_km','var'))
                AltArray_km = 0:50:1000;
            end
            
            % reset to rerun the altitude profile
            this.LAST_ALT_GTD7 = 99999.0; % [km]
            this.LAST_ALT_GTS7 = -999.9;  % [km]
            this.flagVTST7 = 1; % [flag] if 1 then recalculate

            % initialize arrays that hold profile data
            npts = numel(AltArray_km);
            this.ProfileAlt = reshape(AltArray_km,npts,1); % [km] array for generating a profile
            this.D_He = zeros(npts,1);
            this.D_O = zeros(npts,1);
            this.D_N2 = zeros(npts,1);
            this.D_O2 = zeros(npts,1);
            this.D_Ar = zeros(npts,1);
            this.D_Airmass = zeros(npts,1);
            this.D_H = zeros(npts,1);
            this.D_N = zeros(npts,1);
            this.D_OAnom = zeros(npts,1);
            this.D_AirmassWithHotO = zeros(npts,1);
            this.T_neutral = zeros(npts,1);
            this.T_exospheric = zeros(npts,1);
            for ipt = 1:npts
                this.ALT = this.ProfileAlt(ipt);
                [D,T] = this.GTD7();
                this.D_He(ipt) = D(1);
                this.D_O(ipt) = D(2);
                this.D_N2(ipt) = D(3);
                this.D_O2(ipt) = D(4);
                this.D_Ar(ipt) = D(5);
                this.D_Airmass(ipt) = D(6);
                this.D_H(ipt) = D(7);
                this.D_N(ipt) = D(8);
                this.D_OAnom(ipt) = D(9);
                this.T_neutral(ipt) = T(2);
                this.T_exospheric(ipt) = T(1);                
                this.D_AirmassWithHotO(ipt) = 1.66E-24*(4.0*D(1)+16.0*D(2)+28.0*D(3)+32.0*D(4)+40.0*D(5)+D(7)+14.0*D(8)+16.0*D(9));    
            end
            atmTable = table(this.DAY*ones(npts,1),this.SEC*ones(npts,1)/3600.0,this.ProfileAlt, ...
                this.D_O,this.D_N2,this.D_O2,this.D_Airmass, ...
                this.T_neutral,this.T_exospheric, ...
                this.D_He,this.D_Ar,this.D_H,this.D_N,this.D_OAnom, ...
                this.D_AirmassWithHotO);
            atmTable.Properties.VariableNames = {'DOY','UTC','Alt','O','N2','O2','rho','T_neutral','T_exo','He','Ar','H','N','OAnom','rhoWithHotO'};
            atmTable.Properties.VariableUnits = {'day','hr','km','cm-3','cm-3','cm-3','g/cm-3','K','K','cm-3','cm-3','cm-3','cm-3','cm-3','g/cm-3'};
            
            % store inputs with outputs
            atmTable.Properties.UserData.GLAT = this.GLAT;  % [degN]
            atmTable.Properties.UserData.GLONG = this.GLONG; % [degE]
            ...
            % for apparent local time can be included if available but
            % are of minor importance.
            atmTable.Properties.UserData.DAY = this.DAY;           % [doy]
            atmTable.Properties.UserData.SEC = this.SEC;            % [sec UTC]
            atmTable.Properties.UserData.LOCALSOLARTIME = this.LOCALSOLARTIME; % [hr]
            
            % solar wind
            % F107 and F107A values used to generate the model correspond
            % to the 10.7 cm radio flux at the actual distance of the Earth
            % from the Sun rather than the radio flux at 1 AU.
            atmTable.Properties.UserData.F107A = this.F107A; % [sfu]
            atmTable.Properties.UserData.F107 = this.F107;  % [sfu]
            atmTable.Properties.UserData.AP = this.AP;    % [2nT]
    ...
    methods
        ...
        % calculate the densities and temperatures for a given atmospheric
        % layer for the user-defined geographic location, time, and solar
        % wind loading
        
        % GTD7D - This subroutine provides Effective Total Mass Density for
        %         output D(6) which includes contributions from "anomalous
        %         oxygen" which can affect satellite drag above 500 km. 
        [D,T] = GTD7D(this);
        % GTD7  - This subroutine provides Total Mass Density ignoring
        %         contributions from "anomalous oxygen" 
        [D,T] = GTD7(this);
        % thermospheric portion of the above
        [D,T] = GTS7(this,MASS);
        
        % calculate altitude, densities, and temperatures for a given
        % atmospheric layer specified by pressure PRESS [mb]
        [ALT,D,T] = GHP7(this,PRESS);
        
        
        [xGLOB7S] = GLOB7S(this,P);
        [GLOBE7_TINF] = GLOBE7(this,P);
        
        [this] = TSELEC(this,SV);
        [xSCALH] = SCALH(this,ALT,XM,TEMP);
    end
    
    methods(Static)
        [DD,TZ] = DENSM(ALT,D0,XM,MN3,ZN3,TN3,TGN3,MN2,ZN2,TN2,TGN2,RE,GSURF);
        [DD,TZ] = DENSU(ALT,DLB,TINF,TLB,XM,ALPHA,TZ,ZLB,S2,MN1,ZN1,TN1,TGN1,RE,GSURF);
        [REFF,GV] = GLATF(LAT);
        [xCCOR] = CCOR(ALT, R,H1,ZH);
        [xCCOR2] = CCOR2(ALT, R, H1, ZH, H2);
        [xDNET] = DNET(DD,DM,ZHM,XMM,XM);
        [Y2] = SPLINE(X,Y,N,YP1,YPN);
        [YI] = SPLINI(XA,YA,Y2A,N,X);
        [Y] = SPLINT(XA,YA,Y2A,N,X);
        
        % 3hr Magnetic activity functions
        % Eq. A24d
        function val = G0(A,P)
            val=(A-4.0+(P(26)-1.0)*(A-4.0+(exp(-abs(P(25))*(A-4.0))-1.0)/abs(P(25))));
        end
        
        % Eq. A24c
        function val = SUMEX(EX)
            val = 1.0+((1.-EX^19)/(1.-EX))*sqrt(EX);
        end
        
        % Eq. A24a
        function val = SG0(EX,P,AP)
            val = (nrlmsise00.G0(AP(2),P)+(nrlmsise00.G0(AP(3),P)*EX+nrlmsise00.G0(AP(4),P)*EX*EX + nrlmsise00.G0(AP(5),P)*(EX^3) ...
                + (nrlmsise00.G0(AP(6),P)*(EX^4.0)+nrlmsise00.G0(AP(7),P)*(EX^12))*(1.0-(EX^8))/(1.0-EX)))/nrlmsise00.SUMEX(EX);
        end
        
        function val = ZETA(ZZ,Z1)
            val = (ZZ-Z1)*(6367.088132098377173+Z1)/(6367.088132098377173+ZZ);
        end
    ...
    properties(Constant)
        ...
        PT = [ ...
            ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00]';
        % Density of HE O N2 TLB O2 AR H N 'HOT O'
        PD = [[ ...
            1.09979E+00,-4.88060E-02,-1.97501E-01,-9.10280E-02,-6.96558E-03, ...  % HE DENSITY
            ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00]', ...
            [1.02315E+00,-1.59710E-01,-1.06630E-01,-1.77074E-02,-4.42726E-03, ...  % O DENSITY
            ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00]', ...
            [1.16112E+00, 0.00000E+00, 0.00000E+00, 3.33725E-02, 0.00000E+00, ... % N2 DENSITY
            ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00]', ...
            [9.44846E-01, 0.00000E+00, 0.00000E+00,-3.08617E-02, 0.00000E+00, ... % TLB
            ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00]', ...
            [1.35580E+00, 1.44816E-01, 0.00000E+00, 6.07767E-02, 0.00000E+00, ... % O2 DENSITY
            ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00]', ...
            [1.04761E+00, 2.00165E-01, 2.37697E-01, 3.68552E-02, 0.00000E+00, ...% AR DENSITY
            ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00]', ...
            [1.26376E+00,-2.14304E-01,-1.49984E-01, 2.30404E-01, 2.98237E-02, ... % H DENSITY
            ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00]', ...
            [7.09557E+01,-3.26740E-01, 0.00000E+00,-5.16829E-01,-1.71664E-03, ... % N DENSITY
            ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00]', ...
            [6.04050E-02, 1.57034E+00, 2.99387E-02, 0.00000E+00, 0.00000E+00, ... % HOT O DENSITY
        ...
        PTL = [ ...
            [1.00858E+00, 4.56011E-02,-2.22972E-02,-5.44388E-02, 5.23136E-04, ... % TN1(2)
            ...
            [9.39664E-01, 8.56514E-02,-6.79989E-03, 2.65929E-02,-4.74283E-03, ... % TN1(3)
            ...
            [9.85982E-01,-4.55435E-02, 1.21106E-02, 2.04127E-02,-2.40836E-03, ... % TN1(4)
            ...
            [1.00320E+00, 3.83501E-02,-2.38983E-03, 2.83950E-03, 4.20956E-03, ... % TN1(5) TN2(1)
        ...
        PMA = [ ...
            [9.81637E-01,-1.41317E-03, 3.87323E-02, 0.00000E+00, 0.00000E+00, ... % TN2(2)
            ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 2.00000E+00]', ...
            [1.00422E+00,-7.11212E-03, 5.24480E-03, 0.00000E+00, 0.00000E+00, ... % TN2(3)
            ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 2.00000E+00]', ...
            [1.01890E+00,-2.46603E-02, 1.00078E-02, 0.00000E+00, 0.00000E+00, ... % TN2(4) TN3(1)
            ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 2.00000E+00]', ...
            [9.75801E-01, 3.80680E-02,-3.05198E-02, 0.00000E+00, 0.00000E+00, ... % TN3(2)
            ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 2.00000E+00]', ...
            [9.60722E-01, 7.03757E-02,-3.00266E-02, 0.00000E+00, 0.00000E+00, ... % TN3(3)
            ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 2.00000E+00]', ...
            [1.03123E+00,-7.05124E-02, 8.71615E-03, 0.00000E+00, 0.00000E+00, ...  % TN3(4)
            ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 2.00000E+00]', ...
            [1.00828E+00,-9.10404E-02,-2.26549E-02, 0.00000E+00, 0.00000E+00, ... % TN3(5) SURFACE TEMP TSL
            ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 2.00000E+00]', ...
            [1.57293E+00,-6.78400E-01, 6.47500E-01, 0.00000E+00, 0.00000E+00, ... % TGN3(2) SURFACE GRAD TSLG
            ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 2.00000E+00]' ...
            [8.60028E-01, 3.77052E-01, 0.00000E+00, 0.00000E+00, 0.00000E+00, ... % TGN2(1) TGN1(2)
            ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, ...
            0.00000E+00, 0.00000E+00, 0.00000E+00, 0.00000E+00, 2.00000E+00]', ...
            [1.06029E+00,-5.25231E-02, 3.73034E-01, 0.00000E+00, 0.00000E+00, ... % TGN3(1) TGN2(2)
        ...
        % SEMIANNUAL MULT SAM
        % This block data is in the NRLMSISE-00 model, but is not used by
        % the model
        %{
        SAM = [ ...
